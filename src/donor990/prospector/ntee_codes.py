"""Complete NTEE (National Taxonomy of Exempt Entities) code mappings.

Source: National Center for Charitable Statistics (NCCS) at Urban Institute
...
NTEE_CODES = {
    ...
    "Q21": "International Cultural Exchanges",
    "Q22": "International Student Exchange",
    "Q23": "International Exchanges N.E.C.",
    "Q30": "International Development & Relief",
    ...
    "Y25": "State-Sponsored Worker's Compensation Reinsurance",
...
def get_ntee_description(code: str) -> str:
    """Get description for an NTEE code (full or major category)."""
    if not code:
        return "Unknown"
    return NTEE_CODES.get(code.upper(), NTEE_CODES.get(code[0].upper(), "Unknown"))


def get_major_category(code: str) -> str:
    """Get major category letter from an NTEE code."""
    if not code:
        return "Z"
    return code[0].upper()


def get_major_description(code: str) -> str:
    """Get major category description from an NTEE code."""
    return NTEE_MAJOR.get(get_major_category(code), "Unknown")


def codes_match(code1: str, code2: str, level: str = "full") -> bool:
    """
    Check if two NTEE codes match at the specified level.

    Args:
        code1: First NTEE code
        code2: Second NTEE code
        level: Match level - "major" (letter only), "group" (letter + first digit),
               or "full" (exact match)

    Returns:
        True if codes match at the specified level
    """
    if not code1 or not code2:
        return False

    c1, c2 = code1.upper(), code2.upper()

    if level == "major":
        return c1[0] == c2[0]
    elif level == "group":
        # Match on letter + tens digit (e.g., B8x matches B8x)
        return c1[0] == c2[0] and len(c1) > 1 and len(c2) > 1 and c1[1] == c2[1]
    else:  # full
        return c1 == c2
